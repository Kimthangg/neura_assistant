...
import time
from features import tool_create_event, system_prompt_create_event
...
llm_extraction = LLM(system_prompt_create_event, tool_create_event, temperature=0.1)
@pytest.mark.parametrize("prompt, start_date, end_date, start_time, end_time",[
            #Có ngày giờ cụ thể
    ("Tạo sự kiện họp nhóm vào ngày mai lúc 15:00.", tomorrow, tomorrow, "15:00:00", "15:00:00"),
    ("Đặt lịch ăn tối với bạn vào thứ Sáu tuần này lúc 19:30.", (weekdays[4]), (weekdays[4]), "19:30:00", "19:30:00"),
    ("Lên lịch gặp khách hàng vào ngày 25 tháng 3 lúc 10 giờ sáng.", today.replace(day=25,month=3), today.replace(day=25,month=3), "10:00:00", "10:00:00"),
    ...
    ("Lên lịch đi làm căn cước công dân vào ngày 10 tháng 6 lúc 8:00.", today.replace(day=10, month=6), today.replace(day=10, month=6), "08:00:00", "08:00:00"),
    ("Đặt lịch dạy kèm toán cho em tôi vào thứ Sáu từ 18:00 đến 20:00.", (weekdays[4]), (weekdays[4]), "18:00:00", "20:00:00"),
    ("Đặt lịch họp nhóm từ 9 giờ sáng đến 11 giờ trưa ngày 20/04/2025.", today.replace(day=20,month=4), today.replace(day=20,month=4), "09:00:00", "11:00:00"),
    ...
    ("Tạo sự kiện đi du lịch từ ngày 1 đến ngày 5 tháng 8.", today.replace(day=1,month=8), today.replace(day=5,month=8), "00:00:00", "23:59:59"),
    ...
    ("Tạo sự kiện đi dã ngoại với bạn bè từ thứ Bảy đến Chủ Nhật tuần tới.", (weekdays[12]), (weekdays[13]), "00:00:00", "23:59:59"),
    
    #Có địa điểm cụ thể
    ("Tạo lịch đi khám bệnh tại Bệnh viện Bạch Mai vào 9:00 sáng mai.", tomorrow, tomorrow, "09:00:00", "09:00:00"),
    ("Lên lịch hẹn gặp bác sĩ nha khoa ở phòng khám Định Công lúc 10:00 ngày 10/04.", today.replace(day=10, month=4), today.replace(day=10, month=4), "10:00:00", "10:00:00"),
    # ("Tạo sự kiện kiểm tra sức khỏe tổng quát tại Vinmec lúc 7:30 sáng thứ Ba tuần tới.", (weekdays[8]), (weekdays[8]), "07:30:00", "07:30:00"),
    ("Đặt lịch tiêm vaccine tại Bệnh viện Nhi Trung Ương vào 8:00 ngày 15/04.", today.replace(day=15, month=4), today.replace(day=15, month=4), "08:00:00", "08:00:00"),
    ...
    ("Lên lịch đi xem kịch tại Nhà hát Tuổi Trẻ vào 19:00 ngày 10/04.", today.replace(day=10, month=4), today.replace(day=10, month=4), "19:00:00", "19:00:00"),
    # ("Hẹn đi chơi công viên Yên Sở vào 8:00 sáng thứ Bảy.", (weekdays[5]), (weekdays[5]), "08:00:00", "08:00:00"),
    ("Tạo sự kiện tham gia buổi hòa nhạc tại Trung tâm Hội nghị Quốc gia vào 20:00 ngày 20/04.", today.replace(day=20, month=4), today.replace(day=20, month=4), "20:00:00", "20:00:00"),
    # ("Hẹn ăn tối với gia đình tại nhà hàng Sen Tây Hồ vào 19:00 thứ Sáu.", (weekdays[4]), (weekdays[4]), "19:00:00", "19:00:00"),
    ("Lên lịch tổ chức sinh nhật cho em trai tại nhà vào 18:00 ngày 15/04.", today.replace(day=15, month=4), today.replace(day=15, month=4), "18:00:00", "18:00:00"),
    ...
    # ("Lên lịch tham dự hội thảo tại khách sạn Marriott vào thứ Hai tuần này lúc 13:30.", (weekdays[0]), (weekdays[0]), "13:30:00", "13:30:00"),
    # ("Đặt lịch họp kinh doanh tại văn phòng công ty vào thứ Ba tuần này lúc 14:00.", (weekdays[1]), (weekdays[1]), "14:00:00", "14:00:00"),
    # ("Tạo lịch dự buổi thuyết trình tại Đại học Bách Khoa vào thứ Tư tuần này lúc 15:00.", (weekdays[2]), (weekdays[2]), "15:00:00", "15:00:00"),
    # ("Lên lịch tham dự hội thảo tại khách sạn Marriott vào thứ Năm tuần này lúc 13:30.", (weekdays[3]), (weekdays[3]), "13:30:00", "13:30:00"),
    # ("Đặt lịch đi ăn tối với đối tác tại nhà hàng Ngon vào thứ Sáu tuần này lúc 18:00.", (weekdays[4]), (weekdays[4]), "18:00:00", "18:00:00"),
    # ("Đặt lịch đi chơi Vinpearl vào thứ Bảy tuần này từ 9:00 đến 17:00.", (weekdays[5]), (weekdays[5]), "09:00:00", "17:00:00"),
    # ("Đặt lịch dự tiệc cưới tại Trung tâm Tiệc cưới Diamond Palace vào Chủ Nhật tuần này lúc 18:00.", (weekdays[6]), (weekdays[6]), "18:00:00", "18:00:00"),
    
    # # Tuần tới với địa điểm
    # ("Tạo sự kiện hội nghị khách hàng tại Khách sạn Intercontinental từ 8:00 đến 17:00 thứ Hai tuần tới.", (weekdays[7]), (weekdays[7]), "08:00:00", "17:00:00"),
    # ("Đặt lịch tham dự hội chợ việc làm tại Trung tâm Triển lãm vào thứ Ba tuần tới lúc 10:00.", (weekdays[8]), (weekdays[8]), "10:00:00", "10:00:00"),
    # ("Lên lịch tham dự cuộc họp Ban quản trị tại Tòa nhà Landmark 72 vào 9:00 sáng thứ Tư tuần sau.", (weekdays[9]), (weekdays[9]), "09:00:00", "09:00:00"),
    # ("Đặt lịch đi khám sức khỏe định kỳ tại bệnh viện Việt Đức vào thứ Năm tuần tới lúc 8:00.", (weekdays[10]), (weekdays[10]), "08:00:00", "08:00:00"),
    # ("Đặt lịch đi tham quan triển lãm công nghệ tại Trung tâm Hội chợ SECC vào thứ Sáu tuần tới từ 10:00 đến 16:00.", (weekdays[11]), (weekdays[11]), "10:00:00", "16:00:00"),
    # ("Lên lịch đi thăm Bảo tàng Lịch sử vào thứ Bảy tuần sau lúc 14:00.", (weekdays[12]), (weekdays[12]), "14:00:00", "14:00:00"),
    # ("Tạo lịch đi bơi tại Hồ bơi Mỹ Đình vào Chủ Nhật tuần tới lúc 16:00.", (weekdays[13]), (weekdays[13]), "16:00:00", "16:00:00"),
...
def test_extract_event_normal(prompt, start_date, end_date, start_time, end_time):
    ...
    error_file = f"tests/unittest/calendar/log/normal_event_{today_str}.txt"
...
if __name__ == "__main__":
    pytest.main(["-sv", "tests/unittest/calendar/create_normal_event.py"])
# pytest ./tests/unittest/calendar/create_normal_event.py -n auto -v